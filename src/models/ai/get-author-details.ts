...
export async function generateAuthorDetails(name: string, known: AuthorDetails): Promise<AuthorDetails> {
  ...
  const prompt = `Provide biographical details for the author "${name}".${knownContext} Return ONLY a JSON object with keys "birth_year" (number or null) and "bio" (a short 2-3 sentence biography, or null). Only include fields that are not already known above; use null for fields already known. Return ONLY valid JSON, no other text.`;
  ...
  try {
    parsed = await completeText(prompt, {
      maxTokens: 512,
      transform: (rawText) =>
        parseJsonResponse<{ birth_year: number | null; bio: string | null }>(rawText),
    });
    if (isLlmLoggingEnabled()) {
